...
public class DeleteCar implements Operation {
    ...
    public void operation(Database database, JFrame f, User user) {
        ...
        try {
            ResultSet rs0 = database.getStatement()
                    .executeQuery("SELECT `carID`, `Available` FROM `cars`;");
            while (rs0.next()) {
                if (rs0.getInt("Available") < 2) idsArray.add(rs0.getInt("carID"));
            }
        } catch (Exception e0) {
        ...
        confirm.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    ...
                    String carDelete = "DELETE FROM `cars` WHERE `carID` = '" + id.getSelectedItem().toString() + "';";
                    database.getStatement().execute(carDelete);

                    // Delete the car from carregistration table
                    String carRegDelete = "DELETE FROM `carregistration` WHERE `carID` = '" + id.getSelectedItem().toString() + "';";
                    database.getStatement().execute(carRegDelete);
    ...
    private void updateData(String carID) {
        if (carID.equals(" ")) {
            brand.setText("");
            model.setText("");
            color.setText("");
            ...
            try {
                ResultSet rs1 = database.getStatement()
                        .executeQuery("SELECT * FROM `cars` WHERE `carID` = '" + carID + "';");
                rs1.next();
                Car car = new Car();
                car.setID(rs1.getInt("carID"));
