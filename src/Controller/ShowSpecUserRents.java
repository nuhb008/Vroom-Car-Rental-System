    ...
    public void operation(Database database, JFrame f, User user) {
        ...
        try {
            ResultSet rs0 = database.getStatement()
                    .executeQuery("SELECT `userid` FROM `users` WHERE `Type` = '0';");
            while (rs0.next()) {
                ids.add(rs0.getString("userid"));
            }
        ...
        confirm.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                new ShowUserRents(id.getSelectedItem().toString()).operation(database, f, user);
                frame.dispose();
            }
        });
        panel.add(confirm);
