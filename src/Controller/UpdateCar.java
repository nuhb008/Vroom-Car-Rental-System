    ...
    public void operation(Database database, JFrame f, User user) {
        ...
        try {
            ResultSet rs0 = database.getStatement()
                    .executeQuery("SELECT `carid`, `Available` FROM `cars`;"); // Using carid here
            while (rs0.next()) {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    ...
                    String update = "UPDATE `cars` SET `Brand`='" + brand.getText() + "', `Model`='" + model.getText() + "',"
                            ...
                            + "WHERE `carid` = '" + id.getSelectedItem().toString() + "';"; // Using carid here
        ...
        if (ID.equals(" ")) {
            ...
            try {
                ResultSet rs1 = database.getStatement()
                        .executeQuery("SELECT * FROM `cars` WHERE `carid` = '" + ID + "';"); // Using carid here
                rs1.next();
                Car car = new Car();
                car.setID(rs1.getInt("carid")); // Using carid here
